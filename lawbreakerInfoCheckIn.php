...
$json = array();
if (empty($carId) and empty($carOwnerId) and empty($lawbreakerInfo)) {
    $json = array('result' => 'failed', 'errorInfo' => 'Some value is null');
    exit(json_encode($json));
}

$sql = "insert into car_tb (carId, carOwnerId, lawbreakerInfo) VALUES ('{$carId}', '{$carOwnerId}', '{$lawbreakerInfo}')";
